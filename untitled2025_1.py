...
st.title("Predicción de erosión (Y)")
...
pd.set_option('display.max_rows', 100)        # Puedes ajustarlo si quieres más filas
...
plt.title("Evolución de la profundidad Y por abscisa")
...
umbral_minimo = float(input("Ingrese el valor mínimo permitido: "))
...
def resaltar_alertas(row):
    return ['background-color: red; color: white' if row["Alerta"] == "🔴 ALERTA" else '' for _ in row]

# Mostrar como tabla estilizada (solo en Jupyter/Colab)
df_resultado.style.apply(resaltar_alertas, axis=1)

# Ingresar umbral mínimo
umbral_minimo = float(input("Ingrese el valor mínimo permitido: "))
...
for i in range(1, len(fecha_cols)):
    ...
    cambios[f"{mes_actual} ({fecha_anterior} → {fecha_actual})"] = cambio
...
plt.title("¿En qué mes se erosiona más?")
...
# 3. Definir el umbral crítico (puedes cambiarlo)
umbral = 0.6
...
for _, row in df.iterrows():
    ...
    try:
        # Convertir valores a numérico (por si hay errores de formato)
        ...
        # 7. Profundidad actual (última fecha)
        valor_actual = y_vals[-1][0]
        bajo_umbral = "Sí" if valor_actual < umbral else "No"
        ...
        resultados.append({
            ...
            f"¿Ya está bajo {umbral}m?": bajo_umbral,
            f"Fecha cruce umbral {umbral}m": fecha_str
        })

    except Exception as e:
        print(f"⚠️ Error en abscisa {abscisa}: {e}")
        continue
